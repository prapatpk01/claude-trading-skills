...
async function loadFacts(tickers:string[],quotes:Record<string,Quote>){const prices:Object=Object.fromEntries(tickers.map(t=>[t,quotes[t]?.price??null]));const r=await fetch("/api/portfolio-facts",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({tickers,prices})});if(!r.ok)throw new Error("portfolio facts failed");return (await r.json()).facts as Record<string,Fact>;}
...
export default function PortfolioIntelligence({holdings,quotes,lang}:Props){
 ...
 useEffect(()=>{const tickers=tickerKey?TickerArray(tickerKey):[];if(!tickers.length){setAnalysis({});setFacts({});return;}let cancelled=false;setLoading(true);Promise.all([loadMomentum(tickers),loadFacts(tickers,quotes)]).then(([a,f])=>{if(!cancelled){setAnalysis(a);setFacts(f)}}).finally(()=>{if(!cancelled)setLoading(false)});return()=>{cancelled=true};},[tickerKey]); // quotes intentionally excluded: prices are refreshed separately; re-analysis follows holdings changes
