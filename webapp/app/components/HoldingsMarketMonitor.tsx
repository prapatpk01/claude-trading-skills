...
export default function HoldingsMarketMonitor({ onUpdated }: { onUpdated?: () => void }) {
  ...
  return <div className="card holdings-monitor-card" data-feature="unified-technical-v34 holding-reconciliation watchlist-management">
    ...
    <div className="technical-overlay-policy"><strong>{scope === "holdings" ? "TECHNICAL PORTFOLIO OVERLAY" : "TECHNICAL WATCHLIST OVERLAY"}</strong><span>V34 · TREND → FLOW → LOCATION → ACTION</span><small>{scope === "holdings" ? "ADD / HOLD / PROFIT WATCH / TRIM REVIEW / EXIT REVIEW · Technical Target ≠ Fair Value · EXIT always requires thesis/fundamental approval" : "BUY CANDIDATE / WATCH / AVOID / REMOVE REVIEW · Investment must verify thesis, valuation and catalyst before committee review"}</small></div>

    {scope === "watchlist" && <section className="watchlist-manage-panel"><div><strong>ADD WATCHLIST CANDIDATE</strong><small>Adding here creates a research item only—not a BUY order.</small></div><div className="watchlist-manage-grid"><TickerInput value={watchTicker} onChange={value => setWatchTicker(value.toUpperCase())} onSubmitTicker={ticker => setWatchTicker(ticker)} placeholder="Search ticker or company"/><input value={watchReason} onChange={e => setWatchReason(e.target.value)} placeholder="Research reason (optional)"/><button className="btn" disabled={watchSaving || !watchTicker.trim()} onClick={() => void addWatch()}>{watchSaving ? "Adding…" : "+ Add to Watchlist"}</button></div></section>}

    {marketHealth && marketHealth.requested > 0 && <div className={marketHealth.failed.length ? "err" : "notice"} style={{ marginTop: 12 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}><div><strong>MARKET DATA · {marketHealth.complete}/{marketHealth.requested} COMPLETE</strong>{marketHealth.partial.length > 0 && <small style={{ display: "block" }}>Partial history (&lt;220 trading days): {marketHealth.partial.join(", ")}</small>}{marketHealth.failed.length > 0 && <small style={{ display: "block" }}>Provider unavailable: {marketHealth.failed.join(", ")}. No signal is inferred from missing data.</small>}</div>{(marketHealth.failed.length > 0 || marketHealth.partial.length > 0) && <button type="button" className="btn ghost sm" onClick={() => void load()} disabled={loading}>{loading ? "Retrying…" : "Retry market data"}</button>}</div></div>}
    {message && <div className="notice" style={{ marginTop: 12 }}>{message}</div>}

    {editing && <section className="holding-edit-panel"><div className="holding-edit-title"><div><span>EDIT HOLDING · LEDGER SAFE</span><strong>{editing.ticker}</strong></div><button className="btn ghost sm" onClick={() => setEditing(null)}>Cancel</button></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>{(["reconcile", "add", "reduce", "close"] as EditMode[]).map(mode => <button key={mode} className={`btn ${editing.mode === mode ? "" : "ghost"} sm`} disabled={saving} onClick={() => setEditing({ ...editing, mode, shares: mode === "close" ? String(editing.heldShares) : mode === "reconcile" ? String(editing.heldShares) : "" })}>{mode}</button>)}</div><div className="holding-edit-grid"><label><span>Shares</span><input inputMode="decimal" value={editing.shares} disabled={editing.mode === "close"} onChange={e => setEditing({ ...editing, shares: e.target.value })}/></label>{editing.mode === "reconcile" ? <label><span>Average cost</span><input inputMode="decimal" value={editing.avgCost} onChange={e => setEditing({ ...editing, avgCost: e.target.value })}/></label> : <label><span>Trade price</span><input inputMode="decimal" value={editing.tradePrice} onChange={e => setEditing({ ...editing, tradePrice: e.target.value })}/></label>}<button className="btn primary" disabled={saving} onClick={() => void saveEdit()}>{saving ? "Saving…" : editing.mode === "reconcile" ? "Save corrected holding" : "Save"}</button></div></section>}
...
function UnifiedCell({ overlay, item }: { overlay: any; item?: any }) {
  if (!overlay) return <span className="muted">{marketDataNote(item)}</span>;
