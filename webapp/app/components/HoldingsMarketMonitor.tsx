...
export default function HoldingsMarketMonitor({ onUpdated }: { onUpdated?: () => void }) {
  ...
  return <div className="card holdings-monitor-card" data-feature="holding-reconciliation ledger-edit-actions">
    <div className="holdings-monitor-head"><div><h2 className="section" style={{ margin: 0 }}>📈 Holdings Market Monitor</h2><p className="muted" style={{ fontSize: 12, marginTop: 6 }}>Sentinel X sets trend and technical targets; MCDX confirms accumulation/distribution. Weekly decides, Daily times execution.</p></div><span className="tag">{holdings.length} open positions</span></div>
    <div className="technical-overlay-policy"><strong>TECHNICAL PORTFOLIO OVERLAY</strong><span>ADD / HOLD / TRIM / EXIT REVIEW</span><small>Technical Target ≠ Fair Value · MCDX is a synthetic price/volume proxy · EXIT always requires thesis/fundamental approval</small></div>
