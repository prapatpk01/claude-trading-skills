...
function normalizeAction(raw:string,isNew:boolean):Action{
 const a=raw.toUpperCase();
 if(a.includes("TRIM")||a.includes("REDUCE"))return"TRIM";
 if(a.includes("EXIT")||a.includes("SELL"))return"EXIT";
 if(a.includes("REJECT"))return"REJECT";
 if(a.includes("HOLD")||a.includes("WATCH"))return"HOLD";
 if(isNew||a.includes("OPEN")||a.includes("INITIATE"))return"OPEN NEW";
 return"ADD EXISTING";
}

function votesFor(p:Proposal,ctx:any):Ballot{
 const macro=String(ctx?.macroAction??"").toUpperCase();
 const reserve=finite(ctx?.deployable)??0;
 const weight=p.weight??0;
 const positiveAction=p.action==="OPEN NEW"||p.action==="ADD EXISTING";
 const macroVote:VoteLabel=macro.includes("DEFENSIVE")&&positiveAction?"CAUTION":positiveAction?"BUY":"NEUTRAL";
 ...
 const treasuryVote:VoteLabel=positiveAction&&p.amount>reserve?"VETO":positiveAction?"BUY":"NEUTRAL";
 ...
 const votes=[
  ...
  {desk:"TREASURY",vote:treasuryVote,reason:positiveAction?`Deployable liquidity ${usd(reserve)}.`:"No new cash is required for this action."},
 ];
 const buyVotes=votes.filter(x=>supportive(x.vote)).length;
 const vetoed=votes.some(x=>x.vote==="VETO");
 const approved=!vetoed&&(p.action==="TRIM"||p.action==="EXIT"?votes.filter(x=>x.vote==="SELL"||supportive(x.vote)).length>=4:buyVotes>=4);
 return{proposal:p,votes,buyVotes,vetoed,approved};
}
...
export default function CommitteeMeetingV10({lang}:{lang:AppLang}){
 ...
 const proposals=useMemo<Proposal[]>(()=>{
  if(!data)return[];
  const held=new Set((Array.isArray(data.portfolio?.holdings)?data.portfolio.holdings:[]).filter((h:any)=>!h.closed_at).map((h:any)=>String(h.ticker??"").toUpperCase()));
  const out:Proposal[]=[];
 ...
 const approved=ballots.filter(b=>b.approved);
 const toggle=(key:string)=>setSelected(s=>({...s,[key]:!s[key]}));
 const approveAll=()=>setSelected(Object.fromEntries(approved.map(b=>[`${b.proposal.action}:${b.proposal.ticker}`,true])));
 return <section className={styles.room}>
  ...
  <section className={styles.resolutionStats}>{groups.map(g=><div key={g.action}><span>{g.action}</span><strong>{g.items.length}</strong></div>)}</section>
  <section className={styles.portfolioResolution}>
   <div className={styles.resolutionHeader}><div><span>FINAL COMMITTEE RESOLUTION</span><h3>{new Date().toISOString().slice(0,10)} · {ballots.length} proposals</h3></div><button onClick={approveAll} disabled={!approved.length}>Approve All Approved Items</button></div>
   {groups.map(g=>g.items.length>0&&<div className={styles.actionGroup} key={g.action}><h4>{g.action} <span>{g.items.length}</span></h4>{g.items.map(b=>{const key=`${b.proposal.action}:${b.proposal.ticker}`;return <article className={styles.resolutionCard} key={key}><div className={styles.resolutionMain}><div><strong>{b.proposal.ticker}</strong><span>{b.proposal.action}</span><p>{b.proposal.reason}</p></div><div className={styles.amount}><b>{b.proposal.amount>0?usd(b.proposal.amount):b.proposal.weight!=null?`${b.proposal.weight.toFixed(1)}%`:"NO CASH"}</b><small>{b.proposal.currentPrice?`@ ${usd(b.proposal.currentPrice)}`:""}</small></div><div className={b.vetoed?styles.vetoed:b.approved?styles.approved:styles.rejected}>{b.vetoed?"VETOED":b.approved?"APPROVED":"REVIEW"}</div></div><div className={styles.voteGrid}>{b.votes.map(v=><div key={v.desk}><span>{v.desk}</span><b className={styles[voteClass(v.vote)]??""}>{v.vote}</b><small>{v.reason}</small></div>)}</div><div className={styles.cardFooter}><span>{b.buyVotes}/7 supportive votes</span>{b.approved&&<label><input type="checkbox" checked={Boolean(selected[key])} onChange={()=>toggle(key)}/> Human approve</label>}</div></article>})}</div>)}
   {!ballots.length&&<div className={styles.empty}>No portfolio actions are ready for committee resolution.</div>}
  </section>
 </section>
}
