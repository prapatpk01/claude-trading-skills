...
export default function ActiveFundManager({ lang }: { lang: AppLang }) {
  ...
  async function run() {
    ...
    try {
      ...
      const nav = holdings.reduce(
        (s: number, h: any) => s + ((quotes[h.ticker]?.price ?? h.avg_cost) * Number(h.shares)),
        0
      );
  ...
  return (
    <div className="card ai-card" style={{ marginTop: 18 }}>
      ...
      <p className="muted" style={{ fontSize: 12, lineHeight: 1.65 }}>
        {tr(
          lang,
          "This review is not limited to existing holdings. It combines the current portfolio, watchlist, Research ideas, Momentum, Dividend Quality and Thematic scans, then asks the full fund team whether to initiate, add, hold, trim, replace or exit.",
          "การทบทวนนี้ไม่ได้จำกัดเฉพาะหุ้นที่ถืออยู่ ระบบจะรวมพอร์ตปัจจุบัน Watchlist แนวคิดจากทีม Research ผลสแกน Momentum หุ้นปันผลคุณภาพ และหุ้นตามธีม ก่อนให้ทีมกองทุนทั้งหมดประชุมตัดสินใจว่าจะเปิดสถานะใหม่ เพิ่มน้ำหนัก ถือ ลดน้ำหนัก สับเปลี่ยน หรือออกจากการลงทุน"
        )}
      </p>
      <button className="btn" onClick={run} disabled={loading}>
        {loading
          ? tr(lang, "Running full investment committee…", "กำลังประชุมคณะกรรมการลงทุนทั้งทีม…")
          : tr(lang, "🏛 Run Portfolio Opportunity Review", "🏛 เริ่มประชุมค้นหาโอกาสและปรับพอร์ต")}
      </button>
      ...
      {data && (
        <>
          ...
          <IdeaTable ideas={data.newIdeas ?? []} title={tr(lang, "New opportunities — not currently held", "โอกาสลงทุนใหม่ — ยังไม่ได้ถือในพอร์ต")} lang={lang} />
          ...
          {data.replacements?.length ? (
            ...
            <div className="notice">{tr(lang, "No outside idea clears the replacement hurdle. The system will not sell a winner merely to force a rebalance.", "ยังไม่มีหุ้นใหม่นอกพอร์ตที่ผ่านเกณฑ์สับเปลี่ยน ระบบจะไม่ขายหุ้นที่ดีเพียงเพื่อบังคับให้เกิดการ Rebalance")}</div>
          )}

          <h3 className="sub">🏛 {tr(lang, "Fund Operating Process", "กระบวนการทำงานของทีมกองทุน")}</h3>
          <ol style={{ fontSize: 12.5, lineHeight: 1.75 }}>
            {(data.process ?? []).map((x: string, i: number) => <li key={i}>{lang === "th" ? translateProcess(x) : x}</li>)}
          </ol>
          {data.warnings?.length > 0 && <div className="notice">{data.warnings.join(" · ")}</div>}
          <p className="muted" style={{ fontSize: 11 }}>
            {tr(lang, "Decision-support only. Committee proposals do not execute orders automatically.", "ใช้เพื่อสนับสนุนการตัดสินใจเท่านั้น ข้อเสนอของคณะกรรมการจะไม่ส่งคำสั่งซื้อขายโดยอัตโนมัติ")}
          </p>
        </>
      )}
    </div>
...
function MacroPanel({ macro, lang }: { macro: any; lang: AppLang }) {
  return <div className="card" style={{ marginTop: 14, background: "rgba(8,14,28,.48)" }}>
    ...
    <div className="grid cols-3" style={{ marginTop: 10 }}>
      {(macro.scenarios ?? []).map((s: any) => <div className="metric" key={s.name}><div className="label">{lang === "th" ? s.nameTh : s.name} · {s.probability}%</div><div style={{ fontSize: 12, lineHeight: 1.55, marginTop: 6 }}>{lang === "th" ? s.thesisTh : s.thesis}</div></div>)}
    </div>
...
function M({ l, v }: { l: string; v: any }) {
  return <div className="metric"><div className="label">{l}</div><div className="value" style={{ fontSize: 19 }}>{v}</div></div>;
}
...
function translateAction(action: string, lang: AppLang) {
  ...
  const m: Record<string, string> = { INITIATE: "เปิดสถานะใหม่", ADD: "เพิ่มน้ำหนัก", HOLD: "ถือต่อ", "TRIM REVIEW": "ทบทวนการลด", REPLACE: "สับเปลี่ยน", "EXIT REVIEW": "ทบทวนการออก", WATCH: "เฝ้าดู" };
...
function translateProcess(x: string) {
  ...
  if (x.startsWith("Portfolio Construction")) return "ทีมจัดพอร์ตเปรียบเทียบหุ้นใหม่นอกพอร์ตกับหุ้นที่ถืออยู่ทุกตัวจาก Conviction ผลตอบแทนคาดหวัง และต้นทุนค่าเสียโอกาส";
