...
export default function EndToEndInvestmentCommittee({lang}:{lang:AppLang}){
 ...
 return <section className="card" style={{borderTop:"2px solid var(--accent)"}}>
  ...
  <details style={agendaStyle} open><summary style={summaryStyle}>1 · MACRO, REGIME & SENTIMENT</summary><div style={{marginTop:14}}><div className="grid cols-4"><Metric l="Regime" v={meeting.macroLabel}/><Metric l="Macro score" v={`${meeting.macroScore}/100`}/><Metric l="Fund posture" v={meeting.posture}/><Metric l="Target cash" v={`${meeting.cashTarget}%`}/></div><p className="notice"><strong>Macro report:</strong> {data?.macro?.summary??"Market evidence is mixed; deploy only where risk/reward is positive."}</p><Votes rows={meeting.macroVotes}/></div></details>

  <details style={agendaStyle}><summary style={summaryStyle}>2 · PORTFOLIO REVIEW & CAPITAL RELEASE</summary><div style={{marginTop:14}}><p className="muted">Review {meeting.holdings.length} open holdings, propose ADD / TRIM / EXIT, and determine how much capital becomes available.</p><Items items={meeting.portfolioItems} selected={selected} toggle={toggle}/>{!meeting.portfolioItems.length&&<div className="notice">No holding adjustment cleared the portfolio review. Existing holdings remain unchanged.</div>}</div></details>

  <details style={agendaStyle}><summary style={summaryStyle}>3 · INVESTMENT STRATEGY, RESEARCH & CAPITAL ALLOCATION</summary><div style={{marginTop:14}}><p className="muted">Research proposes investable themes and securities. The committee may fund all approved ideas or select only the highest-conviction candidates.</p><Items items={meeting.strategyItems} selected={selected} toggle={toggle}/>{!meeting.strategyItems.length&&<div className="notice">No new strategy candidate passed research, valuation and risk gates. Cash remains in SGOV.</div>}</div></details>

  <details style={agendaStyle}><summary style={summaryStyle}>4 · FINAL RESOLUTION, FUNDING & EXECUTION</summary><div style={{marginTop:14}}><div className="grid cols-4"><Metric l="Approved ideas" v={String(meeting.approved.length)}/><Metric l="Human selected" v={String(selectedItems.length)}/><Metric l="Buy total" v={usd(buyTotal)}/><Metric l="Capital released" v={usd(sellTotal)}/></div><div className="notice" style={{marginTop:12,borderColor:"rgba(72,228,167,.4)"}}><strong>Funding plan:</strong> {funding}</div><div style={{display:"flex",gap:10,flexWrap:"wrap",marginTop:12}}><button className="btn ghost" onClick={selectApproved} disabled={!meeting.approved.length}>Select All Approved</button><button className="btn" onClick={submit} disabled={!selectedItems.length||submitting}>{submitting?"Submitting…":`Submit Rebalance Package (${selectedItems.length})`}</button></div><div className="notice" style={{marginTop:12}}><strong>Meeting minutes:</strong> Macro posture {meeting.posture}. Portfolio actions: {meeting.portfolioItems.filter(x=>selected[key(x)]).map(x=>`${x.action} ${x.ticker} ${usd(x.amount)}`).join(", ")||"none"}. Investment plan: {buys.map(x=>`${x.action} ${x.ticker} ${usd(x.amount)}`).join(", ")||"keep reserve in SGOV"}. Human approval remains mandatory.</div></div></details>
 </section>
}
