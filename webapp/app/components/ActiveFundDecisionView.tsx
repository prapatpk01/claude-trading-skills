...
export default function ActiveFundDecisionView({
  ...
  return <Wrapper className="card ai-card" style={embedded ? undefined : { marginTop: 18 }} data-portfolio-underwriting={embedded ? "same-cio-meeting" : "standalone"}>
    ...
    <p className="muted" style={{ fontSize: 12, lineHeight: 1.65 }}>
      {tr(lang,
        "This is part of the same CIO meeting. Research OS V24 starts with market regime and sector leadership, then runs separate stock engines and the Active Momentum Lifecycle. AM compares every qualified entrant with current holdings before Committee sizing and technical execution.",
        "ส่วนนี้อยู่ใน CIO Meeting เดียวกัน Research OS V24 เริ่มจาก Market Regime และ Sector Leadership แล้วจึงใช้ Stock Engine แยกกันกับ Active Momentum Lifecycle ทีม AM จะเทียบหุ้นใหม่ที่ผ่านเกณฑ์กับ Holdings ทั้งพอร์ตก่อน Committee กำหนดขนาดและ Technical Execution")}
    </p>
    ...
    {data && <>
      ...
      {showResearch && <><DiscoveryEnginePanel discovery={data.discovery} macro={data.macro} lang={lang} /><OpportunityTable rows={data.newIdeas ?? []} lang={lang} /><WatchlistReviewTable rows={data.watchlistReviews ?? []} lang={lang} /><IncompleteResearchTable rows={data.researchIncomplete ?? []} lang={lang} /></>}
      {showPortfolio && <><PortfolioManagementPanel data={data} lang={lang} /><CashPoolPlan plan={data.cashPoolPlan} lang={lang} />{!embedded && <FundingSummary liquidity={data.liquidity} lang={lang} />}<IdeaTable rows={data.existing ?? []} title={tr(lang, "Current holdings — fund state, valuation & technical execution", "หุ้นที่ถืออยู่ — มติกองทุน Valuation และ Technical Execution")} lang={lang} /><ReplacementTable rows={data.replacements ?? []} lang={lang} /></>}
...
function PortfolioManagementPanel({ data, lang }: { data: any; lang: AppLang }) {
  const existing = Array.isArray(data?.existing) ? data.existing : [];
  const entrants = (data?.newIdeas ?? []).filter((row: any) => row.action === "INITIATE");
  const reviews = existing.filter((row: any) => ["TRIM PROFIT REVIEW", "TRIM REVIEW", "EXIT REVIEW"].includes(row.action));
  const replacements = Array.isArray(data?.replacements) ? data.replacements : [];
  return <section className="card" style={{ marginTop: 14, borderTop: "2px solid #9b7cff" }}>
    <span className="tag">AM · ACTIVE PORTFOLIO MANAGEMENT</span>
    <h3 className="sub" style={{ margin: "9px 0 5px" }}>{tr(lang, "Marginal-alpha rebalance board", "กระดาน Rebalance จาก Marginal Alpha")}</h3>
    <p className="muted" style={{ margin: 0, lineHeight: 1.6 }}>{tr(lang, "AM does not wait for a calendar rebalance. Every qualified new idea is compared with every holding; weaker momentum, fading leadership or limited valuation room can become the funding source for a better opportunity.", "ทีม AM ไม่รอวัน Rebalance ตามปฏิทิน หุ้นใหม่ที่ผ่านเกณฑ์ทุกตัวจะถูกเทียบกับ Holdings ทั้งพอร์ต หุ้นที่ Momentum อ่อน Sector เริ่มเสียผู้นำ หรือ Upside ใกล้เต็ม จะเป็นแหล่งเงินให้โอกาสที่ดีกว่า")}</p>
    <div className="grid cols-4" style={{ marginTop: 12 }}>
      <Metric label={tr(lang, "Holdings re-ranked", "Holdings ที่จัดอันดับใหม่")} value={existing.length} />
      <Metric label={tr(lang, "Qualified entrants", "หุ้นใหม่ที่ผ่านเกณฑ์")} value={entrants.length} />
      <Metric label={tr(lang, "TRIM / EXIT reviews", "รายการทบทวน TRIM / EXIT")} value={reviews.length} />
      <Metric label={tr(lang, "Funded rotations", "คู่ Rotation ที่มีแหล่งเงิน")} value={replacements.length} />
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(155px,1fr))", gap: 8, marginTop: 12 }}>
      {[
        ["1 · INV", tr(lang, "Market + sector thesis", "อ่านตลาด + Sector")],
        ["2 · INV", tr(lang, "Rank momentum + upside", "จัดอันดับ Momentum + Upside")],
        ["3 · AM", tr(lang, "Compare with every holding", "เทียบกับทุก Holding")],
        ["4 · AM", tr(lang, "ADD / HOLD / TRIM / EXIT", "เสนอ ADD / HOLD / TRIM / EXIT")],
        ["5 · CIO", tr(lang, "Approve size + execution", "อนุมัติขนาด + Execution")],
      ].map(([stage, label]) => <div className="metric" key={stage}><span>{stage}</span><strong style={{ fontSize: 13, marginTop: 7 }}>{label}</strong></div>)}
    </div>
    <div className="notice" style={{ marginTop: 12 }}><strong>{tr(lang, "Funding order", "ลำดับแหล่งเงิน")}: </strong>{tr(lang, "Approved weak-link rotation first when the marginal-alpha hurdle clears → excess cash above the floor tops up the target weight → no trade if neither source is available.", "ใช้เงินจาก Weak Link ที่อนุมัติเมื่อ Marginal Alpha ผ่านเกณฑ์ก่อน → ใช้เงินสดส่วนเกิน Cash Floor เติม Target Weight → ถ้าไม่มีแหล่งเงินทั้งสองแบบให้คง WATCH")}</div>
  </section>;
}

function DiscoveryEnginePanel({ discovery, macro, lang }: { discovery: any; macro: any; lang: AppLang }) {
  ...
  const leadership = discovery?.marketLeadership ?? {};
  const sectors = Array.isArray(leadership?.sectors) ? leadership.sectors : [];
  const policy = discovery?.holdingPolicy ?? {};
  const signed = (value: unknown) => Number.isFinite(Number(value)) ? `${Number(value) >= 0 ? "+" : ""}${Number(value).toFixed(1)}%` : "—";
  return <section className="card" style={{ marginTop: 14, borderTop: "2px solid var(--accent)" }} data-active-momentum-engines="24">
    <div style={{ display: "flex", justifyContent: "space-between", gap: 12, flexWrap: "wrap", alignItems: "flex-start" }}>
      <div><span className="tag">INV · ACTIVE MOMENTUM RESEARCH V24</span><h3 className="sub" style={{ margin: "9px 0 5px" }}>{tr(lang, "Opportunity map — market first, stocks second", "แผนที่โอกาส — อ่านตลาดก่อน แล้วจึงเลือกหุ้น")}</h3><p className="muted" style={{ margin: 0, maxWidth: 900, lineHeight: 1.6 }}>{discovery?.methodology}</p></div>
      <span className="tag">{discovery?.broadUniverse ?? 0} US → {discovery?.detailedAnalyzed ?? 0} deep dives</span>
    </div>
    <div className="grid cols-4" style={{ marginTop: 12 }}>
      <Metric label={tr(lang, "Macro regime", "สภาวะตลาด")} value={lang === "th" ? macro?.regimeTh ?? "—" : macro?.regime ?? "—"} />
      <Metric label={tr(lang, "Tape sentiment", "Sentiment จากราคา")} value={`${leadership?.sentimentLabel ?? "—"} · ${leadership?.sentimentScore ?? "—"}/100`} />
      <Metric label={tr(lang, "Research focus", "Sector ที่เน้นค้นหา")} value={(leadership?.focusSectors ?? []).slice(0, 3).join(" · ") || "—"} />
      <Metric label={tr(lang, "Risk budget", "Risk Budget")} value={`${macro?.riskBudgetPct ?? "—"}%`} />
    </div>
    <div className="notice" style={{ marginTop: 12 }}><strong>{tr(lang, "Investment Team stance", "มุมมองทีม Investment")}: </strong>{lang === "th" ? leadership?.researchStanceTh ?? "กำลังสร้าง Sector Leadership Map" : leadership?.researchStance ?? "Building sector leadership map"}</div>
    {!!sectors.length && <div className="table-wrap" style={{ marginTop: 12 }}><table className="tbl"><thead><tr><th>#</th><th>{tr(lang, "Sector / proxy", "Sector / ตัวแทน")}</th><th>{tr(lang, "Leadership", "สถานะผู้นำ")}</th><th className="num">Score</th><th className="num">1W</th><th className="num">1M</th><th className="num">3M</th><th className="num">RS 1M vs SPY</th></tr></thead><tbody>{sectors.slice(0, 8).map((row: any) => { const positive = ["LEADING", "IMPROVING"].includes(row.status); const negative = ["FADING", "LAGGING"].includes(row.status); return <tr key={row.etf}><td>{row.rank}</td><td><strong>{row.sector}</strong><small className="muted" style={{ display: "block", marginTop: 3 }}>{row.etf}</small></td><td><strong className={positive ? "pos" : negative ? "neg" : ""}>{row.status}</strong></td><td className="num"><strong>{row.score}/100</strong></td><td className="num">{signed(row.return1w)}</td><td className="num">{signed(row.return1m)}</td><td className="num">{signed(row.return3m)}</td><td className={`num ${Number(row.relative1m) >= 0 ? "pos" : "neg"}`}>{signed(row.relative1m)}</td></tr>; })}</tbody></table></div>}
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(155px,1fr))", gap: 8, marginTop: 12 }}>
      {[
        ["1", tr(lang, "Regime & sentiment", "Regime & Sentiment")],
        ["2", tr(lang, "Sector leadership", "Sector Leadership")],
        ["3", tr(lang, "Momentum lifecycle", "Momentum Lifecycle")],
        ["4", tr(lang, "Fundamental + catalyst", "Fundamental + Catalyst")],
        ["5", tr(lang, "Fair Value room", "Fair Value Room")],
      ].map(([step, label]) => <div className="metric" key={step}><span>INV {step}</span><strong style={{ fontSize: 13, marginTop: 7 }}>{label}</strong></div>)}
    </div>
    <details style={{ marginTop: 12 }}><summary style={{ cursor: "pointer", fontWeight: 700 }}>{tr(lang, "Research engines and evidence ownership", "ดู Research Engine และหลักฐานของแต่ละทีม")}</summary><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(205px,1fr))", gap: 9, marginTop: 10 }}>
      {engines.map((engine: any) => <div className="metric" key={engine.id} style={{ minHeight: 118 }}><span>{engine.role}</span><strong style={{ fontSize: 14, marginTop: 7 }}>{engine.name}</strong><small className="muted" style={{ display: "block", marginTop: 7, lineHeight: 1.45 }}><strong>{tr(lang, "Searches", "ค้นจาก")}: </strong>{lang === "th" ? engine.searchBasisTh ?? engine.searches : engine.searchBasis ?? engine.searches}</small><small style={{ display: "block", marginTop: 8 }}>{tr(lang, "Qualified", "ผ่าน Engine")} · {engine.qualified ?? 0}</small></div>)}
    </div></details>
    {!!rotations.length && <details style={{ marginTop: 12 }}><summary style={{ cursor: "pointer", fontWeight: 700 }}>{tr(lang, "Coverage freshness SLAs — not investment signals", "รอบรีเฟรชข้อมูล 3/7/30/90 วัน — ไม่ใช่สัญญาณซื้อ")}</summary><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(190px,1fr))", gap: 9, marginTop: 10 }}>{rotations.map((window: any) => <div className="metric" key={window.cadence}><span>{window.label}</span><strong style={{ fontSize: 15, marginTop: 6 }}>{window.scheduledThisCycle} {tr(lang, "evidence refreshes", "รายการรีเฟรชหลักฐาน")}</strong><small className="muted" style={{ display: "block", marginTop: 6, lineHeight: 1.45 }}>{window.purpose}</small><small style={{ display: "block", marginTop: 7 }}>{tr(lang, "Next", "รอบถัดไป")} · {String(window.nextRotationAt ?? "").slice(0, 10)}</small></div>)}</div></details>}
    ...
    <div className="notice" style={{ marginTop: 12 }}><strong>{tr(lang, "Mandatory path", "เส้นทางบังคับ")}:</strong> Market/Sentiment → Sector Leadership → Independent Engines → ACCUMULATION / EARLY MARKUP / MOMENTUM EXPANSION → Fair Value gap ≥8% → AM compares Holdings → Committee → Technical Execution → Human Approval.</div>
  </section>;
}
...
function ReplacementTable({ rows, lang }: { rows: any[]; lang: AppLang }) {
  return <><h3 className="sub">🔄 {tr(lang, "AM funded rotation — weaker holding → higher marginal alpha", "AM Funded Rotation — ลดหุ้นที่ด้อยกว่า → เข้าหา Marginal Alpha ที่สูงกว่า")}</h3><p className="muted" style={{ fontSize: 11.5, lineHeight: 1.55 }}>{tr(lang, "A rotation appears only when the entrant clears the required score and remaining-upside edge. AM may recommend a partial rebalance even when cash is available; cash then tops up only the residual target weight.", "จะแสดง Rotation เมื่อหุ้นใหม่มี Portfolio Score และ Upside คงเหลือเหนือหุ้นเดิมตามเกณฑ์ ทีม AM สามารถเสนอ Rebalance บางส่วนแม้ยังมีเงินสด และใช้เงินสดเติมเฉพาะ Target Weight ที่เหลือ")}</p>{rows.length ? <div className="table-wrap"><table className="tbl"><thead><tr><th>{tr(lang, "Source holding", "หุ้นแหล่งเงิน")}</th><th>{tr(lang, "AM action", "มติ AM")}</th><th>{tr(lang, "Destination", "หุ้นทดแทน")}</th><th className="num">{tr(lang, "Funded amount", "วงเงิน Rotation")}</th><th className="num">Score edge</th><th className="num">Upside edge</th><th className="num">Sector edge</th><th>{tr(lang, "Reason", "เหตุผล")}</th></tr></thead><tbody>{rows.map((x: any, i: number) => <tr key={i}><td><strong>{x.from}</strong></td><td><strong className="neg">{x.sourceAction ?? "TRIM REVIEW"}</strong></td><td><strong className="pos">{x.to}</strong></td><td className="num">{money(x.rotateUsd)} · {x.rotatePct}% NAV</td><td className="num">+{Number(x.scoreEdge ?? 0).toFixed(1)}</td><td className="num">{x.expectedReturnEdge == null ? "—" : `${Number(x.expectedReturnEdge) >= 0 ? "+" : ""}${Number(x.expectedReturnEdge).toFixed(1)}%`}</td><td className="num">{x.sectorScoreEdge == null ? "—" : `${Number(x.sectorScoreEdge) >= 0 ? "+" : ""}${Number(x.sectorScoreEdge).toFixed(1)}`}</td><td style={{ fontSize: 11.5, minWidth: 280 }}>{x.reason}</td></tr>)}</tbody></table></div> : <div className="notice">{tr(lang, "No pair clears the marginal-alpha hurdle this cycle. AM keeps current holdings and does not manufacture turnover.", "รอบนี้ยังไม่มีคู่หุ้นที่ผ่าน Marginal Alpha Hurdle ทีม AM จึงคงพอร์ตเดิมและไม่สร้าง Turnover โดยไม่มีเหตุผล")}</div>}</>;
}
