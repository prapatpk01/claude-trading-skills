...
export async function GET(req: NextRequest) {
  ...
  const mode = req.nextUrl.searchParams.get("mode") === "dividend" ? "dividend" : "momentum";
  ...
  try {
    const explicit = raw
      ? raw.split(",").map(t => t.trim().toUpperCase()).filter(t => /^[A-Z.\-]{1,10}$/.test(t)).slice(0, 20)
      : null;

    if (mode === "dividend") {
      const universe = explicit?.length ? explicit : universeForSector(sector);
      return NextResponse.json({
        ...(await runDividendScan(universe, topN)),
        universeSource: explicit?.length ? "explicit" : "sector",
        sector,
        universe,
      });
    }
    ...
    if (explicit?.length) {
      return NextResponse.json({ ...(await runScan(explicit, topN)), mode: "momentum", universeSource: "explicit", sector });
    }
    ...
    if (!thematic) {
      return NextResponse.json({ ...(await runScan(DEFAULT_UNIVERSE, topN)), mode: "momentum", universeSource: "default", sector });
    }
    ...
    await Promise.all(THEME_PROXIES.map(async t => {
    ...
    if (!themed.tickers.length) {
      return NextResponse.json({ setups: [], rejected: [], scanned: 0, sentinel: {}, samp: {}, regime: regime ?? null, playbook, themes: ranked.slice(0, 8), universeSource: "thematic", universeNote: themed.note, warnings: [], sources: ["Daily price history", "Sector and theme proxy ETFs measured against SPY"], mode: "momentum", sector });
    }
