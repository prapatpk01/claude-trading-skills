...
export async function POST(){
 try{
  ...
  const quality=valid.map(p=>{const q=earningsQuality(p.data.financials),ov=p.data.overview;return{t:p.h.ticker,score:q.score??0,margin:ov?.profitMargin==null?null:ov.profitMargin*100,roe:ov?.roe==null?null:ov.roe*100,summary:q.summary};}).sort((a,b)=>b.score-a.score);
  ...
  review.riskRegister.sort((a,b)=>severityRank[a.severity]-severityRank[b.severity]);
