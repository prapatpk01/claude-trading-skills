...
import FundOperatingCycle from "./components/FundOperatingCycle";
...
export default function Home(){
 ...
 return <div className="sentinel-shell">
  ...
  <div className="institutional-layout"><InstitutionalSidebar active={tab} onChange={navigate} lang={lang}/><div className="institutional-content"><InstitutionalPageHeader section={tab} lang={lang}/>{tab==="scanner"&&<OpportunityWorkflow lang={lang}/>}<main className="sentinel-main">
   ...
   {tab==="portfolio"&&<><FundOperatingCycle lang={lang}/><div className="card" style={{marginTop:18}}><div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"center",flexWrap:"wrap"}}><div><h2 className="section" style={{margin:0}}>{lang==="th"?"อนุมัติและบันทึกธุรกรรม":"Approve & Record Transactions"}</h2><p className="muted" style={{margin:"6px 0 0"}}>{lang==="th"?"ใช้หลังตรวจมติจาก Fund Operating Cycle เท่านั้น ระบบไม่ซื้อขายอัตโนมัติ":"Use only after reviewing the Fund Operating Cycle resolution. Nothing executes automatically."}</p></div><HoldingTransactionForm onSaved={()=>setPortfolioRefresh(v=>v+1)}/></div></div><PortfolioTruthSummary lang={lang} refreshKey={portfolioRefresh}/><HoldingsMarketMonitor/><DividendCalendarPanel lang={lang}/><HoldingsIntelligence lang={lang}/><PortfolioLedgerPanel lang={lang} refreshKey={portfolioRefresh}/><CashLedgerPanel lang={lang} refreshKey={portfolioRefresh}/><CashBufferPanel lang={lang} refreshKey={portfolioRefresh}/><PortfolioOptimizerPanel lang={lang} refreshKey={portfolioRefresh}/><OpportunityAllocationPanel lang={lang} refreshKey={portfolioRefresh}/><DividendLedgerPanel lang={lang} refreshKey={portfolioRefresh}/><details className="card" style={{marginTop:18}}><summary style={{cursor:"pointer",fontWeight:800}}>{lang==="th"?"ข้อมูลพอร์ตเดิมและเครื่องมือขั้นสูง":"Legacy portfolio detail & advanced tools"}</summary><div className="portfolio-legacy" key={portfolioRefresh}><PortfolioTab/></div></details></>}
   {tab==="scanner"&&<AlphaScannerV2 lang={lang}/>} 
  </main></div></div>
  <div className="footer-note">{lang==="th"?<>Sentinel Investment OS v10.0 · AI CIO ภายใต้การกำกับของมนุษย์<br/>ระบบไม่ส่งคำสั่งซื้อขายอัตโนมัติ ทุกการตัดสินใจต้องผ่านหลักฐาน Governance และ Audit Trail</>:<>Sentinel Investment OS v10.0 · Institutional AI CIO with mandatory human oversight.<br/>No recommendation executes automatically; every decision requires evidence, governance and an audit trail.</>}</div>
 </div>
}
