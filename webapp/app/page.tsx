...
export default function Home() {
  ...
  return (
    <div className="sentinel-shell sentinel-v12" data-sentinel-version={SENTINEL_RELEASE.appVersion} data-investment-version={SENTINEL_RELEASE.investmentVersion} data-capital-clarity-version={SENTINEL_RELEASE.capitalClarityVersion} data-forecast-version={SENTINEL_RELEASE.forecastVersion} data-technical-version={SENTINEL_RELEASE.technicalVersion} data-research-version={SENTINEL_RELEASE.researchVersion} data-architecture="unified-cio-command-path" data-source-of-truth="portfolio-ledger">
      <header className="sentinel-topbar v11-topbar"><div className="sentinel-brand-lockup" aria-label="Sentinel Investment"><div className="sentinel-mark" aria-hidden="true"><svg viewBox="0 0 72 78" role="img"><defs><linearGradient id="sentinelWing" x1="8" y1="8" x2="65" y2="72" gradientUnits="userSpaceOnUse"><stop offset="0" stopColor="#f2f3ff"/><stop offset=".28" stopColor="#9fc7ff"/><stop offset=".58" stopColor="#317cff"/><stop offset=".82" stopColor="#8f5cff"/><stop offset="1" stopColor="#31d9f3"/></linearGradient></defs><path d="M36 4 63 17 55 27 36 18 17 27 9 17 36 4Z" fill="url(#sentinelWing)"/><path d="M12 25 34 35 34 47 20 41 27 52 34 55 34 73 22 64 8 42 12 25Z" fill="url(#sentinelWing)"/><path d="M60 25 38 35 38 47 52 41 45 52 38 55 38 73 50 64 64 42 60 25Z" fill="url(#sentinelWing)"/></svg></div><div className="sentinel-wordmark"><strong>SENTINEL</strong><strong>INVESTMENT OS</strong><span>Fund Decision OS V{SENTINEL_RELEASE.appVersion} · Momentum Rising</span></div></div><div className="sentinel-control-cluster"><div className="sentinel-status"><span className="sentinel-status-dot"/>MARKET DATA ONLINE</div><button className="btn ghost sm sentinel-lang-btn" type="button" onClick={() => setLang(lang === "en" ? "th" : "en")}>{lang === "en" ? "🇹🇭 ไทย" : "EN"}</button></div></header>
      ...
      <footer className="footer-note v11-footer">{sentinelReleaseLabel} · Momentum Rising V{SENTINEL_RELEASE.investmentVersion} · Capital Clarity V{SENTINEL_RELEASE.capitalClarityVersion} · Research V{SENTINEL_RELEASE.researchVersion} · Holdings Technical V{SENTINEL_RELEASE.technicalVersion} · Forecast V{SENTINEL_RELEASE.forecastVersion} · STATUS → OPPORTUNITIES → PORTFOLIO → CAPITAL → CIO DECISION · Human approval remains mandatory · No automatic execution.</footer>
    </div>
  );
}