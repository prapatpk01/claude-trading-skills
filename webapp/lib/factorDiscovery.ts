...
function scoreOne(a:any):Omit<ResearchCandidate,"status"|"passed"|"gateReasons"|"failedGates">{
 ...
 const rs30=n(tech.rs30)??1;const volRatio=n(tech.volRatio)??1;const upDown=n(tech.upDownVolRatio)??1;const ret1m=n(tech.return1m)??0;const ret3m=n(tech.return3m)??0;
...
function gateReview(mode:FactorMode,row:Omit<ResearchCandidate,"status"|"passed"|"gateReasons"|"failedGates">){
 ...
 if(mode==="momentum")checks.push({label:"Momentum score ≥ 65",pass:row.momentum>=65},{label:"Relative strength ≥ 1.0",pass:(row.metrics.rs30??0)>=1},{label:"Volume ratio ≥ 0.9",pass:(row.metrics.volumeRatio??0)>=.9},{label:"Lifecycle is Accumulation Confirmed / Early Markup / Markup",pass:row.lifecycle.entryEligible});
...
export async function runFactorDiscovery(mode:FactorMode,universe?:string[],topN=10){
 ...
 return {
  mode,version:"23.0-active-momentum-research",asOf:new Date().toISOString(),
