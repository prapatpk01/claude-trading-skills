...
| 1a | ~~`validate:institutional` red on `main`~~ — **fixed.** Four validators asserted superseded page layouts: mounted-component lists that moved one level down into the workspace components, generation numbers pinned to 11.0/12.2/12.3 against a 13.0 page, and label strings that had been reworded | CI never reached `typecheck` or `build`, so nothing downstream of the first validator was ever checked | Done. Every governance contract was kept and several were strengthened against the live approval path; only the arrangement checks were re-pointed. **The dormant components are still on disk and the retire/reconnect decision in §2 is untouched** |
