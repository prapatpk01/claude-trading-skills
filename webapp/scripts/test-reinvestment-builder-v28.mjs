...
const { buildReinvestmentDraft, rankReinvestmentCandidates } = await import(pathToFileURL(policyPath).href);
...
const equal = buildReinvestmentDraft({ deployableUsd: 1688.5, totalNavUsd: 12867, selected: candidates.slice(0, 5), mode: "EQUAL" });
assert.equal(equal.orders.length, 5, "five selected names produce five draft orders when each clears the minimum size");
...
assert.equal(conviction.selectedCount, 8, "builder hard-caps selection at eight names");
...
console.log("Reinvestment Builder V28 position sizing: all assertions passed");
