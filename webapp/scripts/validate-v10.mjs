...
if (!page.includes("v10.2")) failures.push("v10.2 branding is missing");
...
for (const marker of ["PENDING_HUMAN","Approve & record transaction","/api/portfolio","sentinel:portfolio-updated"]) {
...
if (failures.length) {
  console.error("Sentinel v10.2 validation failed:\n" + failures.map((x) => `- ${x}`).join("\n"));
  process.exit(1);
}

console.log("Sentinel Investment OS v10.2 committee state-machine contracts: PASS");
