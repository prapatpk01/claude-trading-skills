...
const cio = fs.existsSync("app/api/v10/cio/route.ts")
  ? fs.readFileSync("app/api/v10/cio/route.ts", "utf8")
  : "";
...
if (!/v10(?:\.0|\.1)/.test(page)) failures.push("v10.x branding is missing");
...
if (failures.length) {
  console.error("Sentinel v10.1 validation failed:\n" + failures.map((x) => `- ${x}`).join("\n"));
  process.exit(1);
}

console.log("Sentinel Investment OS v10.1 release contracts: PASS");
